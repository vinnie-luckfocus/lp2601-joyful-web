...
| 元素 | 色值 | 用途 |
|------|------|------|
| 队徽区域背景 | `#041E42` (MLB Navy) | 渐变背景，从上到下 #041E42 → #0A2A5A |
| 队名文字 | `#FFFFFF` | 白色粗体，28px+ |
| 战绩数字 | `#BF0D3E` (MLB Red) | 胜场/负场数字强调 |
| 简介区域 | `#FFFFFF` | 白色卡片 |
| 成员卡片 | `#FFFFFF` | 白色背景 + 阴影 |
| 在线状态 | `#2D8659` | 绿色圆点 |
| 战术入口按钮 | `#BF0D3E` | 红色填充按钮 |
| 队长标识 | `#C4A35A` (Gold) | 金色皇冠或星星 |

**布局规范**:
- 顶部英雄区：队徽居中（120px），下方队名，再下方战绩
- 球队简介：可展开/收起，默认显示前2行
- 成员网格：4列（桌面），2列（移动端）
- 成员卡片：头像（56px圆形）+ 姓名 + 背号 + 位置

**动效规范**:
- 队徽悬停：轻微放大（scale 1.05）+ 显示简介预览
- 成员列表：依次淡入（stagger 30ms）
- 在线状态点：脉冲动画（呼吸效果）
- 页面滚动：英雄区视差效果（轻微）
