...
以下问题已讨论确定，作为 Skill 实现的约束条件。

### 5.1 总需求到 PRD 的粒度控制
- **拆分原则**：**按功能模块拆**，不按用户故事拆。
- **一个 PRD 对应一个可独立上线的业务功能域**（如用户认证、首页仪表盘、赛程管理）。
- **粒度标准**：不设置严格的行数或 Task 数量上下限。由 `planner` agent 根据模块边界和依赖关系自然划分，**3–8 个 PRD 视为合理区间**。过少说明模块过大，过多说明拆分过细。
- **命名顺序**：PRD 的编号顺序必须严格反映真实依赖关系；`planner` 输出须为带顺序索引的列表，Skill 按索引直接生成文件名。

### 5.2 CCPM 命令的覆盖范围
- **`/pm:epic-decompose` vs `/pm:epic-oneshot`**：
  - **默认使用 `/pm:epic-decompose`**：适用于已通过 `/pm:prd-parse` 生成 `epic.md` 的标准流程。
  - **`/pm:epic-oneshot`** 仅用于快速验证或从空白创建新 Epic 的临时场景。
- **不新增 `/pm:epic-validate`**：Epic 级的构建、测试、类型检查由 Skill **直接调用项目命令**（如 `npm run build`、`npm test`）完成，不封装为新的 CCPM 命令。

### 5.3 自动审核的通过标准量化
- `architect` agent 的输出必须包含结构化结论行：
  ```
  VERDICT: PASS | FAIL
  ```
- 审核采用 **强制 checklist（4 项）**，必须全部通过才算 `PASS`：
  1. **Epic 边界清晰**：每个 Epic 高内聚、低耦合，功能边界不重叠。
  2. **Task 覆盖率 100%**：PRD 中声明的每一个功能点在 Epic/Task 中都有对应实现项。
  3. **依赖顺序一致**：PRD 编号顺序（`01` → `02` → `03`）与功能依赖方向一致，无循环依赖。
  4. **验收标准可测试**：每个 Task 的验收标准包含明确的输入、预期输出和判断条件。
- 任一 checklist 未通过，`VERDICT` 强制为 `FAIL`，Skill 进入修正循环。

### 5.4 失败处理与降级策略
- **遇到合并冲突或 M 轮自动修复仍失败时：立即停止整个流程**，不跳过、不降级继续。
- **Escalation 报告输出位置**：`ccpm/logs/auto-dev-YYYY-MM-DD-HHMMSS-fail.md`，同时摘要写入 `ccpm/auto-dev-state.json` 的 `last_error` 字段。
- **`--skip-epic` 参数**：提供该参数，允许用户在明确知晓风险的情况下，强制跳过当前阻塞的 Epic，继续开发下一个。**跳过的 Epic 编号会被记录在 `ccpm/prds/99-<project>-summary.md` 的 "跳过的 Epic" 列表中。**

### 5.5 跨 Epic 的依赖与冲突
- **允许后续 Epic 修改前置 Epic 已合并的代码**（如数据库表结构调整、接口兼容性增强），但此类修改必须在该 Epic 的 Task 中显式说明。
- **不单独预留"兼容性测试"步骤**：该检查已合并到 **Phase 3 Epic 级验证的"快速回归检查"** 中（运行 `npm run build` + `npm run test:unit`），确保合并到 `main` 后不会破坏已有功能。

### 5.6 日志与可观测性
- **日志持久化**：全流程执行日志写入 `ccpm/logs/auto-dev-YYYY-MM-DD-HHMMSS.log`。每个 Skill 实例对应一个独立日志文件。
- **终端实时反馈**：每个 Epic 开始和结束时输出结构化状态摘要（包含当前进度百分比、耗时、下一步动作）。
- **进度恢复**：`/auto-resume` 启动时优先读取 `ccpm/auto-dev-state.json`，从中恢复中断前的精确位置（当前 PRD、Epic、retry 计数）。
