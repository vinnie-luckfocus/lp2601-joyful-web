...
|------|------|------|
| 视频封面 | 比赛截图 + 暗角 | 深色渐变遮罩 |
| 播放按钮 | `#BF0D3E` (MLB Red) | 红色圆形按钮 |
| 播放图标 | `#FFFFFF` | 白色三角 |
| 时间轴 | `#041E42` (MLB Navy) | 垂直连接线 |
| 精彩时刻点 | `#C4A35A` (Gold) | 金色高亮标记 |
| 视频标题 | `#041E42` | MLB Navy 粗体 |
| 日期/比分 | `#4A5568` | 灰色次要文字 |
| 进度条 | `#BF0D3E` | 红色进度指示 |

**布局规范**:
- 顶部：最新比赛大卡片（全宽，高度 50vh）
- 历史列表：2列网格（桌面），1列（移动端）
- 视频卡片：封面图（16:9）+ 底部信息栏
- 播放页：视频播放器全宽 + 下方时间轴
- 时间轴：垂直线，左侧时间，右侧描述
...
**动效规范**:
- 点击播放：播放按钮扩散消失（scale 2 + opacity 0），视频渐入
- 精彩时刻标记：悬停显示预览缩略图（200ms淡入）
- 时间轴滚动：标记点依次淡入
- 视频切换：淡入淡出过渡（200ms）
- 封面加载：模糊渐显（blur-up lazy loading）
