...
| 元素 | 色值 | 用途 |
|------|------|------|
| 遮罩层 | `rgba(4, 30, 66, 0.7)` | MLB Navy 70%透明 |
| 弹窗背景 | `#FFFFFF` | 白色卡片 |
| Logo | `#041E42` (MLB Navy) | 品牌色 |
| 输入框边框 | `#E2E8F0` → `#BF0D3E` | 默认灰色，聚焦红色 |
| 登录按钮 | `#BF0D3E` → `#A00B34` | 红色填充，悬停加深 |
| 按钮文字 | `#FFFFFF` | 白色粗体 |
| 错误提示 | `#DC2626` | 红色文字 + 图标 |
| 底部文字 | `#4A5568` | 灰色次要文字 |

**布局规范**:
- 弹窗尺寸：最大宽度 400px，最小边距 20px
- 内边距：24px（桌面），16px（移动端）
- 输入框：高度 48px，圆角 8px
- 按钮：高度 48px，全宽，圆角 8px
- Logo：顶部居中，高度 48px

**动效规范**:
- 弹窗打开：从上方滑入（translateY -20px → 0）+ 淡入（200ms）
- 遮罩层：淡入（200ms）
- 关闭：反向动画（150ms）
- 输入框聚焦：边框颜色过渡（200ms）+ 轻微阴影
- 错误提示：抖动动画（shake）+ 红色边框闪烁
- 登录成功：按钮变为加载状态，成功后绿色勾选，弹窗关闭
