...
|------|------|------|
| 数据卡片背景 | `#FFFFFF` | 白色卡片 |
| 关键数字 | `#041E42` (MLB Navy) | 大字体数据 |
| 强调数据 | `#BF0D3E` (MLB Red) | 打击率等重要数据 |
| 次要文字 | `#4A5568` | 标签说明 |
| 趋势图线条 | `#041E42` | MLB Navy 主线条 |
| 趋势图填充 | `rgba(4, 30, 66, 0.1)` | 浅蓝渐变填充 |
| 雷达图填充 | `rgba(4, 30, 66, 0.3)` | 半透明填充 |
| 雷达图顶点 | `#BF0D3E` | 红色顶点标记 |
| Top3 标识 | `#C4A35A` (Gold) | 金色皇冠/星星 |
| 里程碑进度 | `#BF0D3E` → `#041E42` | 红到蓝渐变 |

**布局规范**:
- 顶部：头像（80px）+ 姓名（24px粗体）+ 背号（MLB Red）
- 数据卡片区：3列网格（桌面），2列（平板），1列（手机）
- 卡片内部：大数字（32px+）+ 小标签（12px）
- 图表区：趋势图占60%宽度，雷达图占40%
- 里程碑：底部横向卡片列表

**动效规范**:
- 页面进入：数字从0计数动画到实际值（1000ms，easeOut）
- 趋势图：线条从左到右绘制（800ms）
- 雷达图：顶点依次连接，顺时针绘制（600ms）
- 数据卡片：悬停时轻微上浮 + 阴影加深
- 里程碑达成：金色闪光动画
